...
import os
from context_skills import get_context_skill
...
def analyze_findings(target: str, scan_results: list[dict], target_type: str = "pypi") -> dict:
    ...
    # Inject context skill — gives the model awareness of what it's scanning
    context_skill = get_context_skill(target_type, target)
    context_section = ""
    if context_skill:
        context_section = f"""## Context Skill (Background Knowledge)
{context_skill}

---
"""

    prompt = f"""{context_section}Analyze the following security scan results for a {target_type} package/repository.
